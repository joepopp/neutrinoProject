...
class EventDataset(Dataset):
    def __init__(self, hdf5_file, features, train=True, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        return dataFeatureTensor.float(), dataLabelsTensor.float()
...
class Network(nn.Module):
    def __init__(self, numFeatures, convolve_time=True):
        super().__init__()
        self.conv1 = nn.Conv3d(numFeatures, 32, kernel_size=7, padding=3)
    ...
    def forward(self, x):
        batchSize = x.size()[0]
        x = F.avg_pool3d(self.bn1(F.elu(self.conv1(x))),2)
        x = F.elu(self.conv2(x))
        x = F.avg_pool3d(self.bn2(F.elu(self.conv3(x))),2)
        x = x.view(batchSize, -1)
        x = F.elu(self.lin1(x))
        x = F.elu(self.lin2(x))
        x = F.hardtanh(self.lin3(self.bn3(x)))
        return x
...
#=======================================================================================================================================================
def get_data(inFeatures, trainPercent, valPercent, testPercent, batchSize):
    ...
    for i, file in enumerate(files):
        if (i < trainPercent):
            train_data = EventDataset(file, inFeatures, train=True, transform=MeanNormalize(means,stds))
            train_datasets.append(train_data)
        elif (i < (valPercent+trainPercent)):
            val_data = EventDataset(file, inFeatures, train=False, transform=MeanNormalize(means,stds))
            val_datasets.append(val_data)
        elif (i < (testPercent+valPercent+trainPercent)):
            test_data = EventDataset(file, inFeatures, train=False, transform=MeanNormalize(means,stds))
...
#=======================================================================================================================================================
# Training function
# trains standard model, updates a loss graph throughout training to track progress
# MUST CHANGE OUTPUT FILES
#=======================================================================================================================================================
def training_loop(n_epochs, model, optimizer, loss_fn, train_loader, val_loader, device):
    avg_loss_train = []
    avg_loss_val = []
    best_loss = 1000
    if device==0:
    ...
    try:
        for epoch in range(n_epochs):
            loss_train = 0.0
            for events, locations in train_loader:
                ...
                optimizer.zero_grad()
                loss.backward()
                nn.utils.clip_grad_norm_(model.module.parameters(), 2.5)
                optimizer.step()
            ...
            loss_val = 0.0        
            for events, locations in val_loader:
                with torch.no_grad():
                    #move data to proper device
                    events = events.to(device)
                    locations = locations.to(device)
            ...
            if target_loss < best_loss:
                torch.save({'epoch': epoch+1, 
                            ...
                            'loss': (target_loss)}, 
                            '/data/user/jpopp/training_out/no_time_model_test.pt')
            ...
            plt.xlabel('Epoch')
            plt.legend()
            plt.savefig('/data/user/jpopp/training_out/no_loss_graph_test.png')
...
# MUST CHANGE OUTPUT FILES
#=======================================================================================================================================================
def test(model, parameters, train_loader, val_loader, test_loader, loss_fn):
    ...
    for name, loader in [("Train", train_loader), ("Validate", val_loader), ("Test", test_loader)]:
        with torch.no_grad():
            angles = []
            for events, locations in loader:
                outputs = model(events.float())
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------
#=======================================================================================================================================================
def main(rank, world_size, epochs, inFeatures, dataSplit, lr, batchSize):
    os.environ["MASTER_ADDR"] = "localhost"
    os.environ["MASTER_PORT"] = "12355"
    init_process_group(backend="nccl", rank=rank, world_size=world_size) 
    
    # initialize models, data, optimizer, and loss function
    data = get_data(inFeatures, dataSplit[0], dataSplit[1], dataSplit[2], batchSize)
    ...
    # train model
    training_loop(epochs, model, optimizer, loss_fn, data[0], data[1], rank)
    ...
    if rank == 0:
        ...
        test(model, "/data/user/jpopp/training_out/no_time_model_test.pt", data[0], data[1], data[2], loss_fn)
        print("============================================================================================================")
        # calculate outputs for three events
        check(model, data[3], loss_fn)
        print("---------------------------------------------------------------------------------------------")
        check(model, data[3], loss_fn)
        print("---------------------------------------------------------------------------------------------")
        check(model, data[3], loss_fn)
...
if __name__ == "__main__":
    ...
    # initialize inputs
    epochs = 1000
    inFeatures = ['NPulses','Total Q','Max Q']
    timeFeatures = ['First Time','Max Time','Last Time']
    dataSplit = (15,3,10)
    lr = 0.0013
    batchSize = 100
    
    # run training
    mp.spawn(main, args=(world_size, epochs, inFeatures, dataSplit, lr, batchSize),nprocs=world_size)
